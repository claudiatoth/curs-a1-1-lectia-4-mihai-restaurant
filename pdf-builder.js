...
(function () {
    ...
    function buildTheory() {
        ...
        t = t.replace(/<div class="theory-box"\s+style="background:\s*#F5F0E8[^"]*"[^>]*>/g, '<div class="theory-box warn-box">');
        return `<h1 class="chapter">📘 1. Teorie — Verbul „haben" + sein vs. haben</h1>` + t;
    }
    ...
    function buildExercises() {
        ...
        if (typeof ex1Data !== 'undefined') {
            html += fillInBlock('Übung 1 — Conjugă verbul „haben" (a avea)', 'ich habe · du hast · er/sie/es hat · wir haben · ihr habt · sie/Sie haben.', ex1Data);
        }
        if (typeof ex2Data !== 'undefined') {
            html += fillInBlock('Übung 2 — Cele 3 funcții ale lui „haben"', 'haben = posesie (mașină, casă) · vârstă (ATENȚIE: NU în germană — vârsta cere SEIN!) · stări fizice (Hunger, Durst, Glück).', ex2Data);
        }
        if (typeof ex3Data !== 'undefined') {
            html += fillInBlock('Übung 3 — Sein vs. haben: vârsta (capcană pentru români!)', 'În română: „am 25 de ani". În germană: „ich BIN 25 Jahre alt". Vârsta cere SEIN, NU haben.', ex3Data);
        }
        if (typeof ex4Data !== 'undefined') {
            html += fillInBlock('Übung 4 — Sein pentru profesie + naționalitate', 'Profesia (sunt profesor) și naționalitatea (sunt german) cer SEIN. Fără articol pentru profesie!', ex4Data);
        }
        if (typeof ex5Data !== 'undefined') {
            html += fillInBlock('Übung 5 — Mix sein/haben după regulă', 'Regulă: SEIN pentru ce ești (profesie, naționalitate, adjective). HABEN pentru ce ai (lucruri, stări fizice).', ex5Data);
        }
        return html;
    }
